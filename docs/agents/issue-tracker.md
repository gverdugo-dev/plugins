...
2. **Always label issues of this resource with `resource:plugins`.** It is the only thing that tells the issues of this resource apart from the others. Add it on create and filter by it on list.

## Conventions

- **Create an issue**: `gh issue create -R gverdugo-dev/personal-public-resources --label "resource:plugins" --title "..." --body "..."`. Use a heredoc for multi-line bodies.
...
- **List issues of this resource**: `gh issue list -R gverdugo-dev/personal-public-resources --label "resource:plugins" --state open --json number,title,body,labels,comments --jq '[.[] | {number, title, body, labels: [.labels[].name], comments: [.comments[].body]}]'` with further `--label` and `--state` filters as needed.
...
Create a GitHub issue in `gverdugo-dev/personal-public-resources` labelled `resource:plugins`.
...
Used by `/wayfinder`. The **map** is a single issue with **child** issues as tickets. Everything below happens in the container repo, so `-R gverdugo-dev/personal-public-resources` applies to every command, and both the map and its children carry `resource:plugins`.

- **Map**: a single issue labelled `wayfinder:map`, holding the Notes / Decisions-so-far / Fog body. `gh issue create -R gverdugo-dev/personal-public-resources --label "wayfinder:map,resource:plugins"`.
- **Child ticket**: an issue linked to the map as a GitHub sub-issue (`gh api` on the sub-issues endpoint). Where sub-issues aren't enabled, add the child to a task list in the map body and put `Part of #<map>` at the top of the child body. Labels: `wayfinder:<type>` (`research`/`prototype`/`grilling`/`task`) plus `resource:plugins`. Once claimed, the ticket is assigned to the driving dev.
